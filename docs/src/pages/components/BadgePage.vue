...
// table. Demos are rendered directly from the library's `badgeDemos`, so the
// examples can never drift from the component's real props.
//
// The exported page id (route segment) is `badge`.
import { ref } from "vue";
import { UiBadge, badgeDemos } from "@onyx/vue";
...
const importSnippet = `import { UiBadge } from '@onyx/vue';`;

/** API table rows, ported from the Angular badge.docs.ts metadata. */
interface ApiRow {
  name: string;
  type: string;
  default: string;
...
function demoCode(demo: (typeof badgeDemos)[number]): string {
  const attrs = Object.entries(demo.props ?? {})
    .map(([k, v]) =>
      typeof v === "string" ? `${k}="${v}"` : `:${k}="${JSON.stringify(v)}"`,
    )
    .join(" ");
  const open = attrs ? `<UiBadge ${attrs}>` : `<UiBadge>`;
  return `${open}${demo.slot}</UiBadge>`;
}
  ...
  <article class="docs-page">
    ...
    >
      ...
      <div class="demo__preview">
        <UiBadge v-bind="demo.props">{{ demo.slot }}</UiBadge>
      </div>
