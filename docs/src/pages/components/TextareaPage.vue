...
// table. Demos are rendered directly from the library's `textareaDemos`, so the
// examples can never drift from the component's real props.
//
// The exported page id (route segment) is `textarea`.
import { ref } from "vue";
import { UiTextarea, textareaDemos } from "@onyx/vue";
...
const importSnippet = `import { UiTextarea } from '@onyx/vue';`;

/** API table rows, ported from the Angular textarea.docs.ts metadata. */
interface ApiRow {
  name: string;
  type: string;
  default: string;
...
/** Convert a camelCase prop name to its kebab-case template attribute. */
function kebab(key: string): string {
  return key.replace(/[A-Z]/g, (m) => `-${m.toLowerCase()}`);
}

/** Build a concise, correct Vue usage snippet for a demo. */
function demoCode(demo: (typeof textareaDemos)[number]): string {
  const attrs = Object.entries(demo.props ?? {})
    .map(([k, v]) => {
      // `modelValue` is exposed to consumers as `v-model`.
      if (k === "modelValue") {
    ...
    .join(" ");
  return attrs ? `<UiTextarea ${attrs} />` : `<UiTextarea />`;
}
