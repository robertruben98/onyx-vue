...
// It mirrors the Badge/Button doc page anatomy: title + lead, an import
// snippet, live demos each with a collapsible "show code" panel, and an API
// table. Demos are rendered directly from the library's `tooltipDemos`, so the
// examples can never drift from the component's real props.
//
// The exported page id (route segment) is `tooltip`.
import { ref } from "vue";
import { UiTooltip, tooltipDemos } from "@onyx/vue";


const title = "Tooltip";
const description =
  "Shows a positioned tooltip on hover/focus, hides on leave/blur/Escape, and wires aria-describedby. Built on the overlay primitive.";

const importSnippet = `import { UiTooltip } from '@onyx/vue';`;

/** API table rows, ported from the Angular tooltip.docs.ts metadata and
 * adapted to the Vue component's props/events/slots. */
interface ApiRow {
  name: string;
  type: string;
  default: string;
...
function demoCode(demo: (typeof tooltipDemos)[number]): string {
  const attrs = Object.entries(demo.props ?? {})
    .map(([k, v]) =>
      typeof v === "string" ? `${k}="${v}"` : `:${k}="${JSON.stringify(v)}"`,
    )
    .join(" ");
  const open = attrs ? `<UiTooltip ${attrs}>` : `<UiTooltip>`;
  return `${open}\n  ${demo.slot}\n</UiTooltip>`;
}
  ...
  <article class="docs-page">
    ...
    >
      ...
      <div class="demo__preview">
        <UiTooltip v-bind="demo.props">
          <span class="demo__trigger" v-html="demo.slot" />
        </UiTooltip>
      </div>
