...
// It mirrors the Angular Button doc page anatomy: title + lead, an import
// snippet, live demos each with a collapsible "show code" panel, and an API
// table. Demos are rendered directly from the library's `radioGroupDemos`, so
// the examples can never drift from the component's real props.
//
// The exported page id (route segment) is `radio-group`.
import { ref } from "vue";
import { UiRadioGroup, radioGroupDemos } from "@onyx/vue";
...
const importSnippet = `import { UiRadioGroup } from '@onyx/vue';`;

/** API table rows, ported from the Angular radio-group.docs.ts metadata. */
interface ApiRow {
  name: string;
  type: string;
  default: string;
...
const api: ApiRow[] = [
  ...
  },
  {
    ...
    type: "string",
    default: "''",
    description: "Two-way bound selected value.",
  },
  {
    name: "@valueChange",
...
function demoCode(demo: (typeof radioGroupDemos)[number]): string {
...
// Per-demo selected value so each live preview is independently interactive.
  ...
  <article class="docs-page">
    ...
    >
      ...
      <div class="demo__preview">
        <UiRadioGroup v-model="selected[demo.title]" v-bind="demo.props" />
      </div>
